...
def vtk_writer(
    ...
    field_name: List[str],
    data_type: Literal["scalar", "vector"],
    refVTMName: str,
    save_path_name: str,
    ...
    if data_type not in {"scalar", "vector"}:
        raise ValueError("data_type must be either 'scalar' or 'vector'.")

    fields_per_block = len(field_name)
    field_data_idx = 0
    ...
    for block_i in range(refVTM.n_blocks):
        ...
        end_idx = field_data_idx + fields_per_block
        if end_idx > len(field_data):
            raise ValueError("field_data does not contain enough arrays for all blocks.")

        block_fields = field_data[field_data_idx:end_idx]
        for name, values in zip(field_name, block_fields):
            if data_type == "vector" and is2D:
                vec_3d = np.zeros((block.n_cells, 3), dtype=values.dtype)
                vec_3d[:, :2] = values
                block.cell_data[name] = vec_3d
            else:
                block.cell_data[name] = values
        field_data_idx = end_idx
    ...
    if field_data_idx != len(field_data):
        raise ValueError("field_data contains unused arrays for the provided blocks.")

    output_vtm_file_path = f"{save_path_name}.vtm"
    refVTM.save(output_vtm_file_path, binary=True, compression_level=9)


def truncationErrorCal(singulars: np.ndarray) -> np.ndarray:
    """
...
class PODDataSet:
    ...
    def truncationError(self) -> np.ndarray:
        """
        ...
        """
        if self.singulars_all is None:
            raise ValueError("Full data is not available. Set fullData=True.")
...
class subdomainDataSet:
    ...
    def truncationError(self) -> np.ndarray:
        ...
        """
        if self.singulars_all is None:
            raise ValueError("Full data is not available. Set cal_fullData=True.")
        return truncationErrorCal(self.singulars_all)
