...
class PODImodelAbstract(ABC):
    ...
    def _prepare_predict_input(self, x: np.ndarray) -> np.ndarray:
        """Apply prediction-time preprocessing shared by most models."""
        if x.ndim != 2:
    ...
    def _restore_output_scale(self, y_pred: np.ndarray) -> np.ndarray:
        if self.with_scaler_y:
            return self.scalar_Y.inverse_transform(y_pred)
        return y_pred

    def _finalize_prediction(
        self, y_pred: np.ndarray, reconstruct_pod: bool = False
    ) -> np.ndarray:
        y_pred = self._restore_output_scale(y_pred)
        if reconstruct_pod:
            if self.v is None:
                raise RuntimeError("POD basis is not available. Fit the model first.")
            return y_pred @ self.v
        return y_pred
    ...
    def performPOD(self, y: np.ndarray) -> np.ndarray:
        ...
        """
        if self.v_all is None or self.s_all is None or self.coeffs is None:
            self.reduction(y)
        if self.v_all is None or self.s_all is None or self.coeffs is None:
            raise RuntimeError("POD reduction failed to initialize model state.")
    ...
    def reduction(self, y: np.ndarray) -> None:
        ...
        if self.POD_algo == "svd":
            ...
            sorted_eigenvalues = np.clip(sorted_eigenvalues, a_min=0.0, a_max=None)
            U = U[:, sorted_indices]
    ...
    def truncation_error(self) -> Tuple[float, float]:
        ...
        """
        if self.s_all is None or self.s is None:
